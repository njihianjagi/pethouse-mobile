(async () => {
  ...
  for (const dog of dogs) {
    ...
    const description = data2.querySelector(
      '.breeds-single-content p'
    ).innerText;
    element.description = description;

    let dataHeight = data2.querySelectorAll('.breed-vital-stats-wrapper')[0]
      .children[1].innerText;
    let height = dataHeight.split(' ');
    height[0] = height[0].replace('Height:', '');
    height = height.join(' ');

    let dataWeight = data2.querySelectorAll('.breed-vital-stats-wrapper')[0]
      .children[2].innerText;
    let weight = dataWeight.split(' ');
    weight[0] = weight[0].replace('Weight:', '');
    weight = weight.join(' ');

    let dataLife = data2.querySelectorAll('.breed-vital-stats-wrapper')[0]
    ...
    let dataBreed = data2.querySelectorAll('.breed-vital-stats-wrapper')[0]
